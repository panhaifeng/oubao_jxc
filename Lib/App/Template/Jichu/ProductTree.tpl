	...
	var o = { 
		...
		onnodeclick: function(){
			...
			if(!obj.hasChildren) {
				...
				// alert(parent.document.getElementById('content').src);
				parent.document.getElementById('content').src=parent.document.getElementById('content').src+'&kindId='+obj.id;
			} else {
  	...
  	$(function(){
		...
		$('#btnBlank').click(function(){
			parent.document.getElementById('content').src=parent.document.getElementById('content').src+'&kindId=0';
		});
	});
</script>
<style type="text/css">
...
<div align="right" style="margin-right:20px; clear:both">
      <input type="button" name="btnBlank" id="btnBlank" value=" 清空 ">
</div>  
</body>
</html>