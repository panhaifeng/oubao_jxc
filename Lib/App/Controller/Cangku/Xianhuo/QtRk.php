...
class Controller_Cangku_Xianhuo_QtRk extends Controller_Cangku_Ruku{
    /**
     * 添加登记界面的权限
     * @var array
    */
    ...
    function actionRemove() {
        ...
        $sql="select id from cangku_ruku2product  
        where rukuId=".$_GET['id'];
        $res=$this->_modelExample->findBySql($sql);
        //by 张艳 2015-10-27 添加对码单的判断
        foreach ($res as $key => $value) {
            $sqlto="select m.status from madan_db m
            left join madan_rc2madan x on x.madanId = m.id
            where x.rukuId=".$value['id'];
            $result=$this->_modelExample->findBySql($sqlto);
            foreach ($result as $val) {
                if ($val['status']!= 'active') {
                    js_alert(null,"window.parent.showMsg('码单已锁定，禁止删除')",$this->_url($from));
                    exit;
                }
            }
        }
    ...
    function actionRemoveByAjax() {
        $id=$_REQUEST['id'];
        $m = $this->_subModel;
        $sql="select x.status from madan_db x 
        left join madan_rc2madan y on y.madanId=x.id
        left join cangku_ruku2product c on c.id=y.rukuId
        where c.id=".$id;
        $res=$m->findBySql($sql);
        foreach ($res as $value) {
            if ($value['status'] != 'active') {
                $arr = array('success'=>false,'msg'=>'码单已锁定，禁止删除!');
