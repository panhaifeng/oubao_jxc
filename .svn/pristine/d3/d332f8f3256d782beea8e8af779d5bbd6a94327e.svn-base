...
class Controller_Cangku_Dahuo_Diaohuo extends Controller_Cangku_Xianhuo_Diaohuo{
    ...
    function __construct() {
        ...
        $this->headSon = array(
            ...
            'dahuo2proId' => array(
                ...
                "type" => "BtPopup",
                'name' => 'dahuo2proId[]',
                'url'=>url('Trade_Order','PopupMingxi'),
                'textFld'=>'orderCode',
                'hiddenFld'=>'id',
                'tip'=>'显示大货的订单明细(保存后码单自动锁定)',
                'dialogWidth'=>880
            ),
    ...
    */
    function actionEdit() {
        ...
        foreach($arr['Products'] as & $v) {
            ...
            $sql = "select orderCode from trade_order where id='{$v['dahuo2proId']}'";
            $_temp = $this->_modelExample->findBySql($sql);
            $temp['dahuo2proId']['text']=$_temp[0]['orderCode'];
            $rowsSon[] = $temp;
        }
    ...
    */
    function actionSave(){
        ...
        //2015-10-23 by jiang 如果是调入到现货仓库并且选了订单明细  则保存配货单
        //配货单不能修改和删除  所以修改的时候对配货单不做处理
        if($_POST['kuweiru']=='现货'){
            $this->_savePeihuo($rukuId);
        }
        //跳转
    ...
    */
    ...
    /**
    * ps ：大货调入现货保存配货单
    * Time：2015/10/25 10:36:42
    * @author jiang
    */
    function _savePeihuo($rukuId){
        $_model_peihuo= FLEA::getSingleton('Model_Peihuo_Peihuo');
        $_model_trade= FLEA::getSingleton('Model_Trade_Order2Product');
        $retRuku=$this->_modelRuk->find(array('id'=>$rukuId));
        foreach ($retRuku['Products'] as & $v) {
            $_peihuo=array();
            //选择大货订单才锁定码单和保存配货单
            if($v['dahuo2proId']>0){
                //1、先将码单全部锁定
                $sql="update madan_db set status='lock' where id in (
                    select madanId from madan_rc2madan x 
                    where x.rukuId='{$v['id']}')";
                $this->_modelRuk->findBySql($sql);
            

                //2、保存配货单
                $sql="select x.madanId from madan_rc2madan x where x.rukuId='{$v['id']}'";
                $v['Madan']=$this->_modelRuk->findBySql($sql);
                $retclient=$_model_trade->find(array('id'=>$v['dahuo2proId']));
                $_peihuo = array(
                        'id'=>'',
                        'status_active'=>'未下单',
                        'peihuoDate'=>$retRuku['rukuDate'],
                        'cntM'=>$v['cntM'],
                        'productId'=>$v['productId'],
                        'isDahou'=>1,
                        'clientId'=>$retclient['Order']['clientId'],
                        'order2proId'=>$v['dahuo2proId'],
                        'ruku2proId'=>$v['id'],
                        'Peihuo'=>$v['Madan']
                    );
                $_peihuo['peihuoCode'] = $_model_peihuo->getNewCode();
                $_model_peihuo->save($_peihuo);
            }
        }
    }
