...
class Controller_Dbchange extends TMIS_Controller {
	...
	//根据订单号快速得到相关卷号
	function actionGetQrcodeByOrdercode() {
		$sql = "select a.qrcode from madan_db a
left join ph_peihuo2madan b on a.id=b.madanId
left join trade_order2product c on c.peihuoId=b.phId
left join trade_order d on c.orderId=d.id
where d.orderCode='{$_POST['orderCode']}'";
	// dump($this->m);
		$ret = $this->m->findBySql($sql);
		dump($ret);exit;
		// dump($_POST);exit;
	}
}
