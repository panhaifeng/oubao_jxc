...
<div style=" margin-bottom:5px;">
  <font style="font-weight:bold; font-size:24px; border-bottom:1px double #000;">{$title}</font> 
  <!-- <input type="button" id="button1" name="button1" value="打印" onclick="window_onbeforeprint();prnbutt_onclick();window_onafterprint();" /> -->
  {if $print_href}<a href="{$print_href}" title="打印" style="margin-left:10px;" target="_blank" ><img src="Resource/Image/Print.png" /><span>打印</span></a>{/if}
</div>
{*主表信息,每行显示3个字段*}
{if $arr_main_value}
...
<table id="dataList" width="98%" style="BORDER-COLLAPSE: collapse;" bordercolor="#000000" border="2" align="center" cellpadding="2" cellspacing="0">
  ...
  <tr id="tr">	  	
	{foreach from=$arr_field_info key=key item=item}  
    	{if $key!='_edit'}  		
            ...
            {*word-wrap:break-word;//按单词换行*}
            <td style="width:{$item.width|default:'50px'};word-break:break-all;" >
 ...
 {foreach from=$arr_field_value2 item=field_value2}    
  <div style="display: block;float: left;width: 100%;text-align: left;padding: 10px 10px 10px 20px;">
    {foreach from=$arr_field_info key=key item=item}  
        {if $key!=''}
        <span style="width: 75px;display: block;float:left;font-size: 14px;">{$field_value2.$key}</span>
        {/if}
    {/foreach}  
  </div>    
  {/foreach}      
...
<!-- <input type="button" id="button1" name="button1" value="打印" onclick="window_onbeforeprint();prnbutt_onclick();window_onafterprint();" /> -->
{if $print_href}<a href="{$print_href}" title="打印" style="margin-left:10px;" target="_blank" ><img src="Resource/Image/Print.png" />打印</a>{/if}
</div>
</body>
</html>
