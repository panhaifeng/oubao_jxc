...
function _ctlBtFile($name,$params){
    ...
    if(!$addonPre && !$addonEnd) {
		$html = "<input type='file' class='form-control input-sm' style='width:80px'; name='{$itemName}' id='{$itemName}' {$disabled} {$readonly} />";
    } else {
    	//有头尾附加文字的使用inputgroup
    	$html = '<div class="input-group input-group-sm">';
    	if($addonPre) $html.= '<span class="input-group-addon">'.$addonPre.'</span>';
    	$html.="<input type='file' class='form-control input-sm' name='{$itemName}' id='{$itemName}' {$disabled} {$readonly} />";
