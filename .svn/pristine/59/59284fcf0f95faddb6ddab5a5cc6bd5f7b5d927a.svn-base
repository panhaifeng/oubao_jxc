...
function qtipToCellContents(){
	...
	//自动加载鼠标提示信息
	$('.valueTdDiv').on('mouseover',function(){
...
function thickboxInit(){
	$('body').on('click','.thickbox',function(){
		var url=this.href||this.url;
		var title=this.title?this.title:'';
		var url_data = _GetByUrl(url);
		
		url_data['title']=title;
		title = title!='' ? title : '子窗口';
		var _width = parseInt(url_data['width']) > 0 ? parseInt(url_data['width']) : 650 ;
		var _height = parseInt(url_data['height']) > 0 ? parseInt(url_data['height']) : 440 ;
...
function _GetByUrl(url){
	 var href = url.split("?");
	 if(typeof(href[1]) == "string"){
		var u = href[1].split("&");
		var _get = Array();
		 for(var i in u){
            if(typeof(u[i]) != "string")continue;
			 var j = u[i].split("=");
			 _get[j[0]] = j[1]||'';
		 }
		 return _get;
